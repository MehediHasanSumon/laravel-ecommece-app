...
<div class="container px-6 py-8 mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 mb-6">
         <form id="user-filter-form" action="{{ route('admin.users.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
             <div class="w-full md:w-auto">
                 <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                 <input type="text" name="search" id="search" value="{{ request('search') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="Name or email">
             </div>
             <div class="w-full md:w-auto">
    ...
    <div id="users-table-container" class="bg-white rounded-lg shadow-md overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="#" class="sort-link flex items-center" data-column="email">
                            Email
    ...
    $(document).ready(function() {
        ...
        function loadUsers(url) {
            $.ajax({
                ...
                type: 'GET',
                data: $('#user-filter-form').serialize(),
                beforeSend: function() {
                    // Show loading indicator
                    $('#users-table-container').addClass('opacity-50');
                },
                success: function(response) {
                    // Update the table and pagination
                    $('#users-table-container tbody').html(response.users);
                    $('#pagination-container').html(response.pagination);
                    $('#users-table-container').removeClass('opacity-50');
